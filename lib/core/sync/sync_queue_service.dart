    ...
    await sync.runOutboundStrict(() async {
      switch (entityType) {
        ...
        case 'remito':
          if (operation == 'delete') {
            final numero =
                _payloadString(payloadJson, 'numero') ?? entityId;
            await sync.eliminarRemitoRemoto(numero);
          } else {
        ...
        case 'pedido':
          if (operation == 'delete') {
            final numero =
                _payloadString(payloadJson, 'numero') ?? entityId;
            await sync.eliminarPedidoRemoto(numero);
          } else {
            await sync.subirPedido(int.parse(entityId));
          }
          return;
        case 'documento':
          final map = _decodeMap(payloadJson);
